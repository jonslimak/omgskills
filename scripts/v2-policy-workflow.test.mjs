...
test("scheduled v2 policy enforces shared exclusions and uploads its report before publishing", () => {
  const scrapeStep = workflow.match(
    /      - name: Run scraper\n[\s\S]*?(?=\n      - name:)/,
  )?.[0];
  assert.ok(scrapeStep, "Run scraper step missing");
  assert.match(scrapeStep, /V2_POLICY_MODE: "enforce"/);
  assert.equal(workflow.match(/V2_POLICY_MODE:/g)?.length, 1);
  assert.doesNotMatch(workflow, /V2_POLICY_MODE: "observe"/);
