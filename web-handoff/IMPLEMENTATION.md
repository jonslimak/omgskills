...
- Backend fix `4c3bedb` is deployed. Guarded workflow [36185201880](https://github.com/jonslimak/omgskills/actions/runs/36185201880) verified draft `6ab6d8058da36b7246e2a134`, then production `6ab6d8e9c41bca210f6e25f1`, on the first attempt without rollback. Previous deployment: `6ab6d07e4348cc8766e751f3`.
- Hosted guards passed; app, review, connect, data, library redirects and downloads also passed live checks. All 13 download/update files plus appcast are byte-identical to the baseline. No Mac release, feature-gate or permission changes.
- Repair of the two historical references still awaits a successful Mac sync; deployment alone does not update those rows. No direct production repair was applied, and no database access settings were changed. `/app/review/` remains read-only; test normal `/app/` after syncing.
