# Local Portal Integration (B1-D1)
...
- Home supports account settings, sign-out, handle edits and publication on/off. Sets supports private creation (empty or selected skills), owner details/visibility, Hide/Restore and confirmed deletion. Skills supports membership, public Favorites and sequential bulk adds; owner detail supports add/remove/reorder and email-access management. Agents loads real connected devices and supports confirmed revocation. All writes stay in the isolated database. Favorites identity is protected; connection-code generation and private sources remain disabled.
...
- D1 additionally allows device-list GET and bodyless DELETE for a specific device UUID. No pairing, legacy-token, device exchange, sync upload or private-source routes are enabled. Devices load independently on Agents, stay memory-only and clear on account/page changes.
...
- Ignored operator scripts/config are in `.netlify/portal-integration/`. `setup.mjs` refuses to overwrite an existing database. `server.mts` runs the existing handlers at `127.0.0.1:8888`, using `isIntegrationRequest` and `isIntegrationBody` for the narrow D1 allowlist, including `portal-devices.mts` for list/revocation. It validates the exact database name/data directory through the application's `getPgPool()` and rejects unrelated paths/bodies, unexpected hosts/origins, oversized bodies and non-development Clerk keys. Item/email DELETE must parse its `itemId`/`emailId` body; device DELETE has no body. This is a local handler harness, not verification of Netlify routing/runtime.
