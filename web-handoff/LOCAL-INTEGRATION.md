# Local Portal Integration (B1-C4)
...
- Ignored operator scripts/config are in `.netlify/portal-integration/`. `setup.mjs` refuses to overwrite an existing database. `server.mts` runs the existing handlers at `127.0.0.1:8888`, using `isIntegrationRequest` and `isIntegrationBody` for the narrow C4 allowlist. It validates the exact database name/data directory through the application's `getPgPool()` and rejects unrelated paths/bodies, unexpected hosts/origins, oversized bodies and non-development Clerk keys. Item/email DELETE must parse its `itemId`/`emailId` body. This is a local handler harness, not verification of Netlify routing/runtime.
