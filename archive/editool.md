...
- **Preview is isolated.** “Build local preview” publishes collections and builds
  web-library pages inside an OS temporary directory, verifies them, and serves
  them read-only on `127.0.0.1:8787`. It does not modify `site/data`.
...
Preview artifacts are written outside the repository and are recreated for each
build.
...
- the tool never commits, deploys, or calls the network
- saved policy changes may be inspected with `npm run policy:replay`; Editool
  does not run replay, dispatch CI, or approve an enforcement change
- preview generation uses saved files only and runs the collections publisher,
  web-library generator, and local verifier against an isolated `SITE_DIR`
...
- `index/scripts/editool-preview.ts` — prepares the isolated preview data,
  runs generation and verification, and serves the result read-only.
...
- production deployment or git operations from the UI
